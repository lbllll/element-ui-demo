          ...
          if (res.code == 200) {
            ...
            scene_id =
              scene_id.substring(scene_id.length - 1) == ","
                ...
                : scene_id;
            this.sizeForm.sceneId = scene_id.split(",");
  ...
  methods: {
    ...
    getKey(item, index) {
      ...
      for (let i = 0; i < this.attribute.length; i++) {
        if (this.attribute[i].name.name == item) {
          k = this.useless[i];
          break;
        }
      }
    ...
    checkStock() {
      ...
      for (const k in this.sizeForm.attrs) {
        if (
          ...
          var arr = JSON.parse(JSON.stringify(this.sizeForm.attrs[k]));
      ...
      stockList.forEach(e => {
        ...
        for (const k in e) {
          if (e[k]) {
            var name = this.attribute[this.useless.indexOf(k)].name.name;
    ...
    getAttribute(id) {
      produckAllAttribute({ categoryId: id }).then(result => {
        try {
          ...
          if (isDetail) {
            // 初始化商品信息里的saleAttributes字段，保证该字段与attribute结构一致
            var obj = {},
              arr = JSON.parse(JSON.stringify(this.detailInfo.saleAttributes)),
              newArr = [];
            if (arr && arr.length > 0) {
              arr.forEach(e => {
                obj[e.nameId] = e;
            ...
            result.data.forEach(e => {
              if (obj[e.name.nameId]) {
                newArr.push(obj[e.name.nameId]);
              } else {
                var stock = {
                  dataType: "STOCK",
                  name: e.name.name,
            ...
            this.detailInfo.saleAttributes = newArr;
            attrs();
          }
        } catch (error) {
          console.log(error);
